package com.xnjz.one.util.audioplayer;
...
import com.xnjz.one.exo.CallBackListener;
...
//        }
    ...
    public void startListenProgress() {
        mHandler.post(loadStatusRunnable);
    }
    ...
    public void releasePlayer() {
        ...
        if (checkExoPlayerIsInited()) {
            mHandler.removeCallbacks(loadStatusRunnable);
            mHandler = null;
            exoPlayer.release();
            exoPlayer = null;
            trackSelector = null;
     ...
     * 播放本地音频
     *
     * @param uriName 本地raw目录里的文件名
     */
    public void startLocalRadioWithName(String uriName, CallBackListener callBack) {
        if (TextUtils.isEmpty(uriName)) {
            ToastUtils.showShort("文件名为空");
            return;
        }
        if (checkExoPlayerIsInited()) {
//            int resourceId = mContext.getResources().getIdentifier("qa_four", "raw", mContext.getPackageName());
            int resourceId = mContext.getResources().getIdentifier(uriName, "raw", mContext.getPackageName());
            if (resourceId != 0) {
                // 创建Uri对象
     ...
     * 暂停播放，并清理播放文件
     */
    public void stopRadioAndClearSource() {
        if (checkExoPlayerIsInited()) {
            exoPlayer.clearMediaItems();
            exoPlayer.stop();
        }
...
//        }
    ...
    public Runnable loadStatusRunnable = new Runnable() {
        @Override
        public void run() {
            long durationUs = 0;
            int adGroupCount = 0;
