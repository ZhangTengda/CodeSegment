...
import com.xnjz.one.http3.exception.ApiException
import com.xnjz.one.http3.result.Result
...
sealed class NetworkResponse<out T : Any> {
     ...
     * 业务错误
     */
    data class BizError(val errorCode: Int = 0, val errorMsg: String = "") :
...
fun <T : Any> NetworkResponse<T>.exceptionOrNull(): Throwable? =
    when (this) {
        ...
        is NetworkResponse.BizError -> ApiException(errorCode, errorMsg)
...
fun <T : Any> NetworkResponse<T>.getOrThrow(): T =
    when (this) {
        is NetworkResponse.Success -> data
        is NetworkResponse.BizError -> throw ApiException(errorCode, errorMsg)
